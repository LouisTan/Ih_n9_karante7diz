...
function [block] = quantif_inv(block_quantif,quality)
    assert(numel(block_quantif)>0 && size(block_quantif,1)==8 && size(block_quantif,2)==8 && size(block_quantif,3)==1);
    ...
    closest_quality = quant_mat_qualities(4);
    closest_quant_mat = quant_mats{4};
    ...
    %block = zeros(size(block_quantif));
    block =  double(block_quantif .* int16(closest_quant_mat));
end
